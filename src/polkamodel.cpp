...
void PolkaModel::updateIdentity( const Identity &identity )
{
  int i = 0;
  for( ; i < m_identities.size(); ++i ) {
    if ( m_identities[i].id() == identity.id() ) {
      m_identities[i] = identity;
      break;
    }
  }
  if ( i == m_identities.size() ) {
    m_identities.append( identity );
  }
}
